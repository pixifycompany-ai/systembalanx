...
function SwipeCard({ item, onConfirm, onDelete, onClick }: { item: Despesa; onConfirm: (id: string) => void; onDelete: (id: string) => void; onClick: (d: Despesa) => void }) {
  const [offsetX, setOffsetX] = useState(0);
  const startX = useRef(0);
  const startY = useRef(0);
  const isDragging = useRef(false);
  const isHorizontal = useRef<boolean | null>(null);
  const isSettled = item.status === 'pago';
  const THRESHOLD = 80;
  ...
  const badge = (() => {
    if (item.status === 'pago') return { label: 'Pago', cls: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400' };
    if (item.status === 'atrasado') return { label: 'Atrasado', cls: 'bg-red-500/15 text-red-600 dark:text-red-400' };
    return { label: 'Não pago', cls: 'bg-red-500/15 text-red-600 dark:text-red-400' };
  })();
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="text-[11px] text-muted-foreground truncate">{item.fornecedor || '-'}</span>
            <span className="text-[11px] text-muted-foreground">·</span>
            <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full", badge.cls)}>{badge.label}</span>
          </div>
        </div>
        <span className="text-[15px] font-semibold shrink-0 tabular-nums text-red-600 dark:text-red-400">
...
export function MobileDespesasList({ despesas, isLoading, onEdit, onConfirm, onDelete, onNew }: MobileDespesasListProps) {
      ...
      {grouped.map(([date, items]) => (
        <div key={date}>
          ...
          <div className="space-y-1.5">
            {items.map((d) => (
              <SwipeCard key={d.id} item={d} onConfirm={onConfirm} onDelete={onDelete} onClick={onEdit} />
            ))}
          </div>
        </div>
      ))}
